...
import { Plus, Trash2, Pencil, CreditCard, Banknote, Coffee, ShoppingCart, Car, Heart, Home, Smartphone, Tag } from 'lucide-react';
...
const formasPagamento = [
  ...
  { key: 'Debito', label: 'Débito', icon: <CreditCard size={14} />, cor: '#2563eb', bg: '#eff6ff' },
  { key: 'Credito', label: 'Crédito', icon: <CreditCard size={14} />, cor: '#dc2626', bg: '#fef2f2' },
];

export default function GerenciadorCarteira({ cores, formatarMoeda }) {
  ...
  const handleSalvar = async (e) => {
    e.preventDefault();
    const payload = { descricao, valor: parseFloat(valor) || 0, categoria, forma, data };
    if (editandoId) {
  ...
  const handleEditar = (g) => {
    ...
    setForma(g.forma); setData(g.data || '');
    setEditandoId(g.id);
  };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: '10px', flexWrap: 'wrap', backgroundColor: cores?.branco, padding: '15px', borderRadius: '12px', border: '1px solid #e5e7eb' }}>
        ...
        <div style={{ display: 'flex', gap: '5px' }}>
          {formasPagamento.map(f => (
            <button key={f.key} type="button" onClick={() => setForma(f.key)} style={{ padding: '10px 14px', borderRadius: '8px', border: forma === f.key ? `2px solid ${f.cor}` : '1px solid #ddd', backgroundColor: forma === f.key ? f.bg : '#fff', cursor: 'pointer', fontWeight: forma === f.key ? 'bold' : 'normal', color: f.cor, display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px' }}>
              {f.icon} {f.label}
            </button>
          ))}
        </div>
