...
export default function RelatorioContabil({
  ...
  saldoGlobalConsolidado, valorBensDireitos, totalAtivos, totalPassivos, patrimonioLiquido
}) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div style={{ backgroundColor: '#fff', padding: '20px', borderRadius: '12px', display: 'flex', gap: '20px', flexWrap: 'wrap', boxShadow: '0 4px 15px rgba(0,0,0,0.05)', borderLeft: `4px solid ${cores?.primaria}` }}>
        <div style={{ flex: '1 1 150px' }}><label style={{fontSize:'12px', fontWeight:'bold', color:'#666'}}>Ano Ficheiro</label><br/><select value={anoContabil} onChange={e=>setAnoContabil(e.target.value)} style={{width:'100%', padding:'10px', borderRadius:'6px', border:'1px solid #ccc'}}><option value="Todos">Historico Completo</option><option value="2027">2027</option><option value="2026">2026</option><option value="2025">2025</option></select></div>
        <div style={{ flex: '1 1 150px' }}><label style={{fontSize:'12px', fontWeight:'bold', color:'#666'}}>Mes Referencia</label><br/><select value={mesContabil} onChange={e=>setMesContabil(e.target.value)} style={{width:'100%', padding:'10px', borderRadius:'6px', border:'1px solid #ccc'}}><option value="Todos">Anual Completo</option><option value="2026-06">Junho/26</option><option value="2026-07">Julho/26</option><option value="2026-08">Agosto/26</option></select></div>
        <div style={{ flex: '1 1 150px' }}><label style={{fontSize:'12px', fontWeight:'bold', color:'#666'}}>Responsavel</label><br/><select value={perfilContabil} onChange={e=>setPerfilContabil(e.target.value)} style={{width:'100%', padding:'10px', borderRadius:'6px', border:'1px solid #ccc'}}><option value="Todos">Familia (Consolidado)</option>{perfis.map(p=><option key={p.id} value={p.id}>{p.nome}</option>)}</select></div>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '25px' }}>
        <div style={{ backgroundColor: '#fff', padding: '30px', borderRadius: '12px', borderTop: '4px solid #17a2b8', boxShadow: '0 4px 15px rgba(0,0,0,0.05)' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '20px', marginTop: '15px', backgroundColor: resultadoExercicio >= 0 ? '#d4edda' : '#f8d7da', borderRadius: '8px' }}>
            <span style={{ fontWeight: 'bold', fontSize:'16px' }}>Resultado do Exercicio</span>
        ...
        <div style={{ backgroundColor: '#fff', padding: '30px', borderRadius: '12px', borderTop: '4px solid #6f42c1', boxShadow: '0 4px 15px rgba(0,0,0,0.05)' }}>
          <h3 style={{ margin: '0 0 25px 0', display: 'flex', alignItems: 'center', gap: '8px', color:'#6f42c1' }}><Scale size={24}/> Balanco Patrimonial (Global)</h3>
          <h4 style={{ margin: '10px 0 10px 0', color: '#6f42c1', fontSize: '14px' }}>ATIVOS (Bens e Dinheiro)</h4>
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0' }}><span style={{fontSize:'14px'}}>Caixa e Bancos</span><strong>{formatarMoeda(saldoGlobalConsolidado)}</strong></div>
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0' }}><span style={{fontSize:'14px'}}>Imobilizado (Imoveis, Veiculos)</span><strong>{formatarMoeda(valorBensDireitos)}</strong></div>
          ...
          <h4 style={{ margin: '20px 0 10px 0', color: '#dc3545', fontSize: '14px' }}>PASSIVOS (Obrigacoes Futuras)</h4>
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0' }}><span style={{fontSize:'14px'}}>Contas Pendentes (Lancadas)</span><strong>{formatarMoeda(totalPassivos)}</strong></div>
